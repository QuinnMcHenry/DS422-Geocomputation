...
```{r}
num_fires <- wildfire_shp_data %>% 
  group_by(ACQ_DATE) %>% 
  summarize(nums = n())
ggplot(data = num_fires, aes(ACQ_DATE, nums)) + geom_line()
```
...
ggplot(wildfire_shp_data, aes(x = ACQ_DATE, fill = CONFIDENCE)) +
  geom_bar() +
  labs(
    x = "Date",
    y = "Number of Fires",
...
library(dplyr)
library(ggplot2)
df <- read.csv(here("LANDSAT_USA_contiguous_and_Hawaii_24h.csv"))
head(data)

ggplot() +
  borders("world", colour = "gray50", fill = "gray95") + 
  geom_tile(data = df, aes(x = longitude, y = latitude, fill = as.factor(path)), color = "white") +
  geom_point(data = df, aes(x = longitude, y = latitude), color = "red", size = 2) + 
  labs(
    title = "Sensor Tile Map by Path",
    subtitle = "Acquisition Date: 2025-09-17",
...
fires <- st_read(here("data/LANDSAT_USA_contiguous_and_Hawaii_7d/LANDSAT_USA_contiguous_and_Hawaii_7d.shp"))

glimpse(fires)

fires_filtered <- fires %>%
  filter(ACQ_DATE == as.Date("2025-09-17"))
...
# NASA or NOAA are great sources. Much easier to do in Python, in my humble opinion. The biggest challenge, unless you already have programs running to identify what times of the day you will get good passes over the area you want to see, is the sheer volume of imagery produced by a single satellite (scans every 15 seconds, all day, every day) and sorting through these without having an idea of how to check shapefiles / work with h5/netcdf can be overwhelming. 
# to confirm predictions, we can use historical data to see if similar fires behaved in similar ways to how our plots predict. or use my website  http://fireaq.alaska.edu/GEOS-FP-LEAF.html to see if air quality predictions match up with where we think fires are moving, air quality / aerosol concentration will be worse with smoke
