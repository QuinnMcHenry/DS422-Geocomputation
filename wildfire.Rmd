...
#tile_width <- 0.2  # degrees longitude
#tile_height <- 0.2 # degrees latitude

ggplot() +
  borders("world", colour = "gray50", fill = "gray95") + 
  geom_tile(data = df, aes(x = longitude, y = latitude, fill = as.factor(path)),
             color = "white") +
  geom_point(data = df, aes(x = longitude, y = latitude), color = "red", size = 2) + 
  scale_fill_brewer(palette = "Set2", name = "Path") +
  labs(
